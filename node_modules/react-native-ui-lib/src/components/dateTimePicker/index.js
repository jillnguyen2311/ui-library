...
const DateTimePicker = forwardRef((props, ref) => {
  ...
  return <>
    ...
    renderCustomOverlay={Constants.isAndroid ? renderAndroidDateTimePicker : undefined} testID={`${testID}.overlay`}>
      ...
      // @ts-expect-error
      ref={textField} migrate={migrateTextField} testID={testID} editable={editable}
      // @ts-expect-error should be remove after completing TextField migration
      expandable={migrateTextField ? undefined : !!others.renderExpandableInput} value={getStringValue()} />}
      </ExpandableOverlay>
    </>;
});
